###  Overview of the changes made, feature-wise
- Trivial record constructors removed/simplified
- Trivial fields getters removed
- Added a few unit tests to test non-trivial getters, primary and secondary constructors
- A new CLI argument, `rrm` (Remove Record Methods) was added to enable/disable the feature. Enabled by default.

###  Overview of the changes made, code wise
Inside the method `isSyntheticRecordMethod`, we check for constructors without annotations, and field getters that are not implemented by the user.
If the function is deemed be generated by javac (basic `this.<field> = <field>` stuff), it is considered a synthetic record and gets committed from the final code.


From the `methodToJava` perspective, in the case of a trivial primary record constructor that has annotations, the code content of the constructor is removed, but we keep the declaration of said constructor.
`// Now that's I'm writing this, I wonder if this creates an issue to recompile stuff.`

Some miscellaneous functions were added, mostly to make things easier to read.

###  Thought process of resolving the task
Thanks to the already provided tests I went in steps, from the simple test that only had default functions before moving on to more complex cases. This can be seen through me making multiple commits.
Since I knew the tests were limited in scope, I added a few tests that I knew would force to rework my code to support a wider array of cases.

### Final notes
I know for a fact there are some cases that are probably not covered by my testing.
Some things could have been done to improve readability further, like "de-propagate" annotations back to the record declaration, in order to remove trivial getters/primary constructors that are targeted by said annotations.
